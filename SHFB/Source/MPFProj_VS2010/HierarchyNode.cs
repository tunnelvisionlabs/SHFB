...
//=============================================================================
// File    : HierarchyNode.cs
// Updated : 04/19/2011
// Modifier: Eric Woodruff  (Eric@EWoodruff.us)
//
// This file has been modified to support "Add as Link" in the project.  Search
// for "!EFW" to find the changes.
//
// NOTE: Other changes added support for linked files.  Drag and drop and
// cut/copy/paste do work with the linked files.  However, unlike a regular C#
// project, if a linked file is dragged, cut, or copied, when dropped or pasted
// it will become an actual file in the project in the new location rather than
// remaining as a link.  I couldn't figure out a way to maintain the link state
// in the drop/paste handling code since that info doesn't carry over in the
// clipboard data.
//
//    Date     Who  Comments
// ============================================================================
// 06/18/2008  EFW  Fixed bug in DragEnter.  Added notes on linked files.
// 03/20/2011  EFW  Updated to use MPFProj for VS2010
// 04/04/2011  EFW  Added support for "Show All Files" and Refresh Solution
//=============================================================================
	...
	//, IVsBuildStatusCallback 
	{
		...
		public virtual object GetProperty(int propId)
		{
			...
			switch((__VSHPROPID)propId)
			{
                ...
                case __VSHPROPID.VSHPROPID_IsNonMemberItem:     // !EFW
		...
		public virtual int SetProperty(int propid, object value)
		{
			...
			switch(id)
			{
                ...
                case __VSHPROPID.VSHPROPID_IsNonMemberItem:     // !EFW
                    ...
                    {
                        // This also resets the Exclude From Source Control state too
                        this.ExcludeNodeFromScc = this.IsNonMemberItem = parsedValue;
		...
		{
            // !EFW
            // Supress edit for non-member items
            if(this.IsNonMemberItem)
                return null;
		...
		public virtual void Remove(bool removeFromStorage)
		{
            ...
            // !EFW - Handle exclude from project when Show All Files is toggled on by converting the node
            // to a non-member item.
            bool isLinked = !String.IsNullOrEmpty(this.ItemNode.GetMetadata(ProjectFileConstants.Link));
            bool convertToNonMemberNode = (!removeFromStorage && !isLinked && this.ProjectMgr.ShowingAllFiles &&
                (File.Exists(this.Url) || Directory.Exists(this.Url)));

			// Notify hierarchy event listeners that the file is going to be removed.
            if(!convertToNonMemberNode)
			    this.OnItemDeleted();
            ...
            if(!convertToNonMemberNode)
            {
                ...
                if(!this.IsNonMemberItem)
                {
                    ...
                    // Convert this node to a non-member node
                    this.IsNonMemberItem = true;
                    this.itemNode.RemoveFromProjectFile();
                    this.itemNode = new ProjectElement(this.ProjectMgr, null, true);
        ...
        // !EFW
        /// <summary>
        /// This is overridden to enable the "Add as Link" functionality in
        /// the Add Existing Item dialog.
        /// </summary>
        /// <param name="addType">The type of item to add</param>
        /// <returns>Returns <c>VSConstants.S_OK</c> on success or an error
        /// code on failure.</returns>
        /// <remarks>Note that this just enables the dropdown on the Add
        /// button.  The actual support for linked files in the project takes
        /// more work and is implemented in the ProjectNode and FileNode
        /// classes.</remarks>
        protected virtual int AddItemToHierarchy(HierarchyAddType addType)
        {
            ...
            string strBrowseLocations = Path.GetDirectoryName(
                this.projectMgr.BaseURI.Uri.LocalPath);

            System.Guid projectGuid = this.projectMgr.ProjectGuid;

            addItemDialog = this.GetService(typeof(IVsAddProjectItemDlg)) as
            ...
            if(addType == HierarchyAddType.AddNewItem)
                ...
                uiFlags = (uint)(__VSADDITEMFLAGS.VSADDITEM_AddExistingItems |
                    __VSADDITEMFLAGS.VSADDITEM_ProjectHandlesLinks |    // !EFW - The magic value for "Add as Link"
                    __VSADDITEMFLAGS.VSADDITEM_AllowMultiSelect |
                    __VSADDITEMFLAGS.VSADDITEM_AllowStickyFilter);

            ErrorHandler.ThrowOnFailure(addItemDialog.AddProjectItemDlg(
                this.hierarchyId, ref projectGuid, project, uiFlags, null,
                null, ref strBrowseLocations, ref strFilter,
		...
		protected virtual int ExecCommandOnNode(Guid cmdGroup, uint cmd, uint nCmdexecopt, IntPtr pvaIn, IntPtr pvaOut)
		{
			...
			else if(cmdGroup == VsMenus.guidStandardCommandSet97)
			{
				...
				switch((VsCommands)cmd)
				{
                    ...
                    case VsCommands.Refresh:    // !EFW - Added support for Refresh
                        this.ProjectMgr.RefreshProject();
                        return VSConstants.S_OK;
				}
			}
			else if(cmdGroup == VsMenus.guidStandardCommandSet2K)
			{
				...
				{
                    case VsCommands2K.INCLUDEINPROJECT:     // !EFW - Added support for Include in Project
                        FolderNode fn = this as FolderNode;

                        if(fn != null)
                            return fn.IncludeInProjectRecursive();
                    ...
                    case VsCommands2K.SLNREFRESH:   // !EFW - Added support for Refresh
                        this.ProjectMgr.RefreshProject();
                        return VSConstants.S_OK;
                }
			}
		...
		/// <summary>
		/// Executes commands that are independent of a selection.
		/// </summary>
		/// <param name="cmdGroup">Unique identifier of the command group</param>
		/// <param name="cmdId">The command to be executed.</param>
		/// <param name="cmdExecOpt">Values describe how the object should execute the command.</param>
		...
		{
            // !EFW - Added support for Refresh
            if(cmdGroup == VsMenus.guidStandardCommandSet97 && (VsCommands)cmd == VsCommands.Refresh)
            {
			...
			{
                // !EFW - Added support for Show All Files and Refresh Solution
                switch ((VsCommands2K)cmd)
                {
				    case VsCommands2K.SHOWALLFILES:
					    result |= QueryStatusResult.SUPPORTED | QueryStatusResult.ENABLED;

                        if(this.projectMgr.ShowingAllFiles)
        ...
        public static IEnumerable<HierarchyNode> FindNodes(HierarchyNode root,
          Func<HierarchyNode, bool> predicate)
        {
            if(root == null)
                throw new ArgumentNullException("root");
