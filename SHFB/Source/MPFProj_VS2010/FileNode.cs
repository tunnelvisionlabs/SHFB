...
// the project.  Search for "!EFW" to find the changes.  Note that these
...
namespace Microsoft.VisualStudio.Project
{
    ...
    public class FileNode : HierarchyNode
    {
        ...
        // !EFW
        /// <summary>
        /// This is overridden to get the overlay icon for linked items
        /// </summary>
        /// <param name="propId">The property ID to get</param>
        /// <returns>The property ID value</returns>
        public override object GetProperty(int propId)
        {
            if((__VSHPROPID)propId == __VSHPROPID.VSHPROPID_OverlayIconIndex &&
              !String.IsNullOrEmpty(this.ItemNode.GetMetadata(ProjectFileConstants.Link)))
        ...
        /// <exception cref="InvalidOperationException">Returned if the file cannot be validated</exception>
        /// <devremark> 
        ...
        // !EFW
        /// <summary>
        /// This is overridden to suppress renaming on linked items
        /// </summary>
        /// <returns>Returns the node caption if editing is allowed or null
        /// if editing is not allowed</returns>
        ...
        /// <p />This allows for dragging to a node that cannot contain
		...
		protected virtual void RenameInStorage(string oldName, string newName)
		{
            ...
            else
            {
                ...
                // so just adjust the link.
                if(Path.GetFileName(oldName) != Path.GetFileName(newName))
                {
                ...
                {
                    newName = PackageUtilities.MakeRelativeIfRooted(newName, this.ProjectMgr.BaseURI);
