...
>
	<!--
	TODO: Rework syntax and code section handling in this file and CodeSnippet.xslt and move it back into the
	presentation style.

	The whole code snippet handling part is much too complex and hard to maintain within the XSL transformations.
	It also makes it hard to customize the language tabs displayed and their order without modifying the
	transformations directly.  With regard to branding, this probably was the only choice.  However, by moving the
	processing back into the presentation style, we have much better options.  Planned changes:

	1. Extend the SyntaxComponent configuration dialog to allow the order of the syntax sections to be defined
		 based on the user's preferences.  Also allow the user to indicate which languages should be grouped together
		 in a tabbed section for those presentation styles that support it.

	2. The SyntaxComponent can be extended to locate consecutive code blocks and reorder them based on the
	   order defined for the syntax sections.  Code blocks for languages not represented in the syntax section
		 and those that are standalone will be rendered as single blocks that are always visible.  Code blocks
		 related to the syntax section languages will be rendered as a tabbed section and will be shown or hidden
		 based on the selected tab.

	3. The rendering of the code blocks in a tabbed section or standalone block will be moved back into the
	   presentation style.  Since block ordering is handled by the SyntaxComponent, rendering them should become
	...
	<xsl:template match="mtps:CodeSnippet" priority ="2" name="codeSnippetOverride">
		<xsl:choose>
	...
	<xsl:template name="renderCodeDiv">
		...
		<xsl:element name="div" namespace="{$xhtml}">
			...
			<xsl:variable name="spacedSnippetCode">
				<xsl:apply-templates select="msxsl:node-set($snippetCode)/node()" mode="codeSpacing"/>
			</xsl:variable>
			<xsl:choose>
								...
								<xsl:choose>
									<xsl:when test="$plain-remove-element!='' or $plain-remove-id!='' or $plain-remove-class!=''">
										<xsl:variable name="plainSnippetCode">
											<xsl:apply-templates select="msxsl:node-set($spacedSnippetCode)" mode="plainCode"/>
										</xsl:variable>
										<xsl:value-of select="cs:plainCode($plainSnippetCode)" disable-output-escaping="yes"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="cs:plainCode($spacedSnippetCode)" disable-output-escaping="yes"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:element>
						</xsl:when>
