...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl"
                ...
                xmlns:mtps="http://msdn2.microsoft.com/mtps"
                         xmlns:col="urn:mtps-colorize"
                         xmlns:tok="urn:mtps-tokenize"
...
>
  ...
  <!--<xsl:key name="all-code-snippets" match="//mtps:CodeSnippet[ancestor::mtps:collapsibleArea]" use="@language"/>-->
  ...
  <xsl:template name="code-snippet-menu">
    ...
    <xsl:element name="a" namespace="{$xhtml}">
      <xsl:attribute name="id">
        <xsl:value-of select="concat($id, '_ViewColorized')"/>
      </xsl:attribute>
      <xsl:attribute name="href">
        <xsl:text>#</xsl:text>
      </xsl:attribute>
      <xsl:attribute name="onclick">
        <xsl:text>javascript:ExchangeTitleContent('</xsl:text>
      ...
      <xsl:attribute name="title">
        <xsl:value-of select="'View Colorized'"/>
      </xsl:attribute>
      <xsl:attribute name="style">
        <xsl:value-of select="'display: none'"/>
      </xsl:attribute>
      <xsl:value-of select="$ViewColorizedText"/>
    </xsl:element>
  ...
  <msxsl:script language="C#" implements-prefix="cs">
    ...
    <!--<msxsl:assembly href="C:\Windows\Microsoft.NET\Framework64\v2.0.50727\System.Web.dll" />-->
    <![CDATA[
